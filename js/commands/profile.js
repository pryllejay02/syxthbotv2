...
function getWeaponDisplay(player) {
  ...
  const weaponQuality = equippedWeapon
    ? equippedWeapon.quality === "Starter"
      ? "🌱 Starter"
      : `${equippedWeapon.qualityEmoji || getQualityEmoji(equippedWeapon.quality)} ${
          equippedWeapon.quality
        }`
    : "🌱 Starter";
...
module.exports = async function profileCommand(message) {
  ...
  await persistRebalancedPlayer(playerRef, player, rebalancedPlayer).catch(
    (error) => {
      console.error("Profile rebalance save failed:", error);
    }
  );

  const level = Number(rebalancedPlayer.level ?? 1);
  const exp = Number(rebalancedPlayer.exp ?? 0);
  const hp = Number(rebalancedPlayer.hp ?? 100);
  ...
  const playerClass = rebalancedPlayer.class || "Novice";
  const worldName = rebalancedPlayer.world?.name || "No World Selected";
  ...
  if (hp <= 0) {
    const remainingSeconds = getReviveRemainingSeconds({
      ...rebalancedPlayer,
      hp,
      maxHp,
    });
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `${revivedNotice}` +
        `**Adventurer:** ${rebalancedPlayer.username || message.author.username}\n` +
        `**Class:** ${rebalancedPlayer.classEmoji || "⚔️"} ${playerClass}\n` +
